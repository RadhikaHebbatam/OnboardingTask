...
--b)Current Home Value for all properties Listed above
 ...
 SELECT QR.RentPayment,
 QR.Frequency,
 QR.StartDate AS TenantStartDate,
 QR.EndDate AS TenantEndDate,
 QR.TotalRent As AllPayments,
 ((QR.AnnualReturn/QR.PropertyValue)*100) AS Yield 
 FROM
(
  SELECT   OP.PropertyId ,   
	                 P.Name as PropertyName ,
		             ...
		             END) AS TotalRent,
					 (CASE TPF.Name
			         WHEN 'Weekly' THEN (TP.PaymentAmount * 52)
				     WHEN 'Fortnightly' THEN (TP.PaymentAmount * 26)
				     ELSE (TP.PaymentAmount * 12)
		             END) AS AnnualReturn,
